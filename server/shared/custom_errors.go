...
var (
	...
	ErrSubtaskNotCompleted   = &CustomError{"all subtask status must be completed if task status is completed", http.StatusBadRequest}
	ErrInvalidTaskCond       = &CustomError{"task must be completed if all subtask status are completed", http.StatusBadRequest}
	...
	ErrInvalidParam          = &CustomError{"id parameter must be numeric and bigger than zero", http.StatusBadRequest}
)
