...
type TaskRepository interface {
	...
	UpdateDue(ctx context.Context) func()
...
func (r taskRepository) Find(ctx context.Context, queries map[string]string) ([]model.Task, int64, int64, error) {
	...
	for k, v := range queries {
		...
		case "end":
			dbQuery = dbQuery.Where("created_at <= '?'::date", v)
		case "status":
			dbQuery = dbQuery.Where("status LIKE ?", v)
		case "sortBy":
...
func (r taskRepository) Update(ctx context.Context, task model.Task) (model.Task, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		log.Println(task.Deadline, finderTask.Deadline)
		if !task.Deadline.IsZero() {
			finderTask.Deadline = task.Deadline
		}

		if task.Subtasks != nil {
			log.Println(task.Subtasks)
			...
			if err := tx.Model(&model.Subtask{}).Where(model.Subtask{TaskID: task.ID}).Update("status", task.Status).Error; err != nil {
				tx.Rollback()
				return shared.ErrSubtaskUpdateFailed
			}
		}
	...
	return finderTask, err
}

func (r taskRepository) UpdateDue(ctx context.Context) func() {
	return func() {
		currentTimestring := time.Now().Format(shared.CompareLayoutFormat)
		r.db.WithContext(ctx).Model(&model.Task{}).Where("status = ?", shared.OngoingStat).Where("deadline::date <= ?::date", currentTimestring).Update("status", shared.DueStat)
	}
}
